...
export default function SpinPage() {
  ...
  async function loadOutcomes() {
    ...
    // The public view already filters out inactive and unavailable outcomes.
    // It does not expose an `active` column, so filtering it by active causes
    // Supabase to reject the request and leaves the wheel stuck loading.
    let result = await supabase
      .from("public_wheel_outcomes")
      .select("id,label,prize_id")
      .order("created_at", { ascending: true });

    // Fallback for local databases that have not applied the public-view migration yet.
    if (result.error) {
